...
if not exists(
    ...
    where
    	...
    	and p.proname = 'tr_ins_upd_products')
then
...
IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE NOT tgisinternal AND tgname = 'tr_insert_update')
THEN
    --Триггер на вставку и обновление для таблицы products
    create trigger tr_insert_update before insert or update on public.products for each row execute procedure tr_ins_upd_products();
END IF;
